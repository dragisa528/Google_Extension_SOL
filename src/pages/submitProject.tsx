...
const SubmitProject =()=>{
  ...
  const [isAgree,setIsAgress] = useState<boolean>(false);
  ...
  const handleSubmit = ()=>{
    ...
    if(title === "" || description === "" || budget === 0 || power === 0 || city ==="" || country ==="" || state==="" || link ==="" ){
      toast.warn("Please input correctly");
      return;
    }
    if(!isAgree) {
      toast.warn("You dont agree with this Terms and Conditions");
      return;
    }
  ...
  return(
       <div className="bg-white">
          ...
          <div className="grid lg:grid-cols-12 md:grid-cols-8 sm:grid-cols-1 gap-2">
              <input type="text" onChange={(e:any)=>setTitle(e.target.value)} className="bg-[#f2f4f8] rounded-md p-2 border-0 lg:col-span-6 md:col-span-6" placeholder="Enter Project title " />
              <input type="text" onChange={(e:any)=>setLink(e.target.value)} className="bg-[#f2f4f8] rounded-md p-2 border-0 lg:col-span-6 md:col-span-6" placeholder="Enter video link" />
              <input type="text" className="bg-[#f2f4f8] rounded-md p-2 border-0 lg:col-span-2 md:col-span-2" onChange={(e:any)=>setCity(e.target.value)} placeholder="City" />
              <input type="text" className="bg-[#f2f4f8] rounded-md p-2 border-0 lg:col-span-4 md:col-span-5" onChange={(e:any)=>setState(e.target.value)} placeholder="State/Province" />
              <input type="text" className="bg-[#f2f4f8] rounded-md p-2 border-0 lg:col-span-2 md:col-span-3" onChange={(e:any)=>setCountry(e.target.value)} placeholder="Country" />
          </div>
          <textarea name="detail" id="project_detail" onChange={(e:any)=>setDescription(e.target.value)}  cols={30} rows={10} placeholder="Tell us more about your project:"></textarea>
          <div className="flex gap-3">
              <input type="number" className="bg-[#f2f4f8] rounded-md p-2 border-0 flex-3" onChange={(e:any)=>setBudget(parseInt(e.target.value))} placeholder="Budget in USD" />
              <input type="number" className="bg-[#f2f4f8] rounded-md p-2 border-0 flex-3" onChange={(e:any)=>setPower(parseInt(e.target.value))} placeholder="Power: kWh" />
              <div className="flex items-center ml-5">
                  <Checkbox onChange={(e)=>{setIsAgress(e.target.checked)}} className="p-4"/>
                  <a href="#" className="ml-4 text-blue-500">
                  I agree with SolGuerilla Terms and Conditions
                  </a>
              </div>
